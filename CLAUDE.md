...
├── soulport/
│   ├── __init__.py               # 版本号
│   ├── cli.py                    # Click CLI (12 命令)
│   ├── core.py                   # 核心逻辑 (export/absorb/merge/diff/changelog)
│   ├── semantic_merge.py         # 四层过滤 pipeline (file→section→line→LLM)
│   ├── doctor.py                 # 灵魂健康检查 (五层诊断 + 评分)
│   ├── mcp_server.py             # MCP Server (6 tools, FastMCP, stdio+HTTP)
│   ├── watcher.py                # watch 守护进程 + 快照管理
│   ├── scanner.py                # 工作区扫描 (5层文件发现)
│   ├── manifest.py               # 包元数据 (manifest.json, lineage)
│   └── llm_config.py             # LLM 配置管理
└── README.md
```

总代码量: ~3500 行，10 个 Python 文件。
...
- [x] 选择性 absorb 交互 - `--interactive` / `-i` 逐层确认
- [x] 修 glob 匹配 - `_matches_pattern` 重写，`**` 递归通配正确处理（11/11 测试）
...
### v0.6.1 ✅ 四层过滤 pipeline + 打磨 (2026-03-27~28) — 架构升级 + PyPI v0.6.1
- [x] Layer 1: 文件级 identical 过滤（移入 semantic_merge_packages）
- [x] Layer 3: `_classify_diff()` 行级分析（pure_append/tiny/medium/large）
- [x] LLM prompt: `DIFF_MERGE_PROMPT` 用 unified diff（prompt 缩小 10×）
- [x] 更新专利 Claim 13→22 claims：四层过滤方法（file→section→line→LLM）+ dry-run + 阈值配置
- [x] glob `_matches_pattern` 重写 + `?` 通配符支持（11/11 测试）
- [x] `absorb --interactive` 逐层选择
- [x] 小龙虾 Review 6 项全清（8.5→9.0）
- [x] PyPI v0.6.0 + v0.6.1 发布
